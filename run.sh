...
dataset_dir=/data/qq-28076393/data-9 # 数据集目录，这里是写死的，记得修改
...
# This is redundant on tinymind. Because every execution has a new directory.  
# rm -rvf $output_dir/*

# 因为dataset里面的东西是不允许修改的，所以这里要把config文件复制一份到输出目录
cp $DIR/$config $pipeline_config_path

for i in {0..50}  # for循环中的代码执行5此，这里的左右边界都包含，也就是一共训练500个step，每100step验证一次
do
    echo "############" $i "runnning #################"
    last=$[$i*500]
    current=$[($i+1)*500]
